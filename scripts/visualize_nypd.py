...
def plot_crime_age_severity(input_csv: str):
    ...
    for i, case_type in enumerate(case_types):
        ...
        values = [
            subset[subset["Age_Group"] == age]["Count"].sum()
...
def plot_nypd_borough_pie(input_csv: str):
    ...
    plt.title(
        "Annual NYPD Complaint Distribution by Borough"
    )
...
if __name__ == "__main__":
    ...
    plot_nypd_monthly_by_borough(
        "data/nypd_monthly_data.csv"
    )

    plot_crime_age_severity(
        "data/age_counts.csv"
    )

    plot_crime_age_gender(
        "data/gender_age_counts.csv"
    )

    plot_nypd_borough_pie(
        "data/nypd_monthly_data.csv"
    )
